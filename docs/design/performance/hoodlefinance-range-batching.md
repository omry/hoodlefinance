...
status: Draft
updated: 2026-04-11
summary: Planned reimplementation target for array-aware execution and batching in the newer pipeline.
---
...
This note describes the intended reimplementation target for three linked behavior areas:
...
- the newer pipeline should route all inputs through one shared batch pipeline; a scalar call is just a `1x1` batch
- any failing cell aborts the formula with one native Sheets error

## Current Context

The older Apps Script runtime already has array/range behavior and some range-aware batching in `hoodlefinance.js`.

That work has taken a step backward in the newer pipeline and should be treated as a historical baseline rather than the current design target for the migrated core.

This note therefore describes the behavior that should be re-established when range-aware execution is implemented cleanly in the newer pipeline.

## Target Design

- `HOODLEFINANCE` should normalize every call into a 2D job grid:
...
- The runtime should split scalar attribute coercion from ticker-range normalization:
...
- The array path should use a per-invocation execution context object:
...
- One reusable batch utility should handle HTTP fanout:
  ...
  - use an explicit source concept for batching rather than ad hoc per-call fanout
  - batching utility code must not be duplicated, but source-specific fetchers are allowed
- Source-specific fetchers should sit on top of that shared utility:
...
- Attribute extraction should remain source-compatible:
...
- Call-level validation should stay in place:
...
- User-facing docs may continue to describe the legacy runtime behavior until the newer pipeline reaches parity.

## Target Public API / Behavior

- `HOODLEFINANCE("NASDAQ:GOOG", "price")` remains unchanged
- `HOODLEFINANCE(A3:A, "price")` should spill one result per input row
- `HOODLEFINANCE(A3:C10, "isin")` should return an output grid with the same shape
