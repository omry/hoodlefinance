...
function buildAddonDeploymentConfigSource(targetName) {
  const normalizedTarget = normalizeAddonTargetName(targetName);
  const isStaging = normalizedTarget === "staging";

  return [
    "// Generated by tools/deploy-addon.js during workspace preparation.",
    "var HF_IS_ADDON_STAGING = " + (isStaging ? "true" : "false") + ";",
    "",
  ].join("\n");
}
...
async function prepareWorkspace(layout, target, options) {
  ...
  await fsp.writeFile(
    ...
    buildAddonDeploymentConfigSource(normalizedOptions.targetName || DEFAULT_TARGET_NAME),
    "utf8"
  );
